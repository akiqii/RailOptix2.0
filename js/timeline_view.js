...
class TimelineView {
  ...
  renderStrategicGantt(filteredRequests) {
    ...
    // Compute density per week for heatmap tinting matching reference dashboard
    const weekDensity = {};
    for (let w = 1; w <= 30; w++) {
      let count = 0;
      filteredRequests.forEach(r => {
        if (r.scheduled_accesses && r.scheduled_accesses.some(a => a.week === w)) count++;
      });
      if (w >= 12 && w <= 14) {
        weekDensity[w] = 'col-tint-high highlight-col';
      } else if (count >= 10) {
        weekDensity[w] = 'col-tint-med';
      } else {
        weekDensity[w] = 'col-tint-low';
      }
    }

    for (let w = 1; w <= 30; w++) {
      const isSelectedWeek = (this.activeStartWeek && (w === this.activeStartWeek || w === this.activeStartWeek + 1));
      html += `<th class="gantt-header-cell ${weekDensity[w]} ${isSelectedWeek ? 'selected-week-col' : ''}">W${w}</th>`;
    }
    ...
    filteredRequests.forEach(r => {
      ...
      for (let w = 1; w <= 30; w++) {
        ...
        html += `
          <td class="gantt-bar-cell ${weekDensity[w]} ${isSelectedWeek ? 'selected-week-cell' : ''}" 
              data-activity="${r.activity_id}" 
              data-week="${w}">
            ${cellContent}
          </td>
